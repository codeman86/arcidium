...
const FALLBACK_TITLE_FORMATTER = new Intl.DateTimeFormat('en-US', {
  month: 'short',
  day: 'numeric',
  year: 'numeric',
});
...
export function AdminEditorShell({
  ...
  const [titleManuallyEdited, setTitleManuallyEdited] = React.useState<boolean>(
    () => Boolean(article?.title)
  );
  const lastAutoTitleRef = React.useRef<string | null>(null);
  ...
  React.useEffect(() => {
    ...
    initialSlugRef.current = initialDraft.slug;
    setIsDirty(false);
    setStatus({
      type: 'idle',
      message: article ? 'Loaded article.' : 'Ready to draft.',
  ...
  React.useEffect(() => {
    setTitleManuallyEdited(Boolean(article?.title));
    lastAutoTitleRef.current = article?.title ?? null;
  }, [article]);
  ...
  React.useEffect(() => {
    ...
    async function convert() {
      try {
        ...
        if (!cancelled) {
          setPreviewHtml(
            "<p class='text-destructive'>Preview unavailable.</p>"
          );
        }
      } finally {
        if (!cancelled) {
      ...
      try {
          ...
          typeof nextSearchTimestamp === 'number'
            ? (() => {
                ...
                if (
                  ...
                  nextSearchTimestamp <= lastTimestamp
                ) {
                  return false;
                }
        ...
        setRecentSaves((current) => {
          ...
          const previousLocation =
            existingIndex >= 0 ? current[existingIndex].location : undefined;
    ...
    (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      ...
      updateDraft((prev) => {
        ...
        if (field === 'title' && typeof nextValue === 'string') {
          ...
          setTitleManuallyEdited(trimmed.length > 0);
          if (trimmed.length === 0) {
  ...
  const handleMarkdownChange = React.useCallback(
    (value: string) => {
      updateDraft((prev) => {
        ...
        if (!titleManuallyEdited) {
          const autoTitle = deriveAutoTitle(value, lastAutoTitleRef.current);
          const shouldApplyAutoTitle =
            autoTitle &&
            (!prev.title.trim() || prev.title === lastAutoTitleRef.current);
          if (shouldApplyAutoTitle) {
            nextTitle = autoTitle;
    ...
    [titleManuallyEdited, updateDraft]
  );
...
function createInitialDraft(
  article: AdminEditorShellProps['article']
): DraftState {
  return {
    slug: article?.slug ?? '',
    title: article?.title ?? '',
...
function deriveAutoTitle(markdown: string, previousAuto: string | null) {
  ...
  return previousAuto ?? buildFallbackTitle();
}
...
function buildFallbackTitle(date = new Date()) {
  return `New Article — ${FALLBACK_TITLE_FORMATTER.format(date)}`;
}
